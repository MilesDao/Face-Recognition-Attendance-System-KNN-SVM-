...
A total of \textbf{306 face images} were extracted from 22 videos. Table~\ref{tab:extract_summary} shows the per-student distribution.
...
Observation: The number of images per class is relatively balanced (ranging from 9 to 24), which motivates the need for data augmentation.
...
After augmentation, the dataset grew from \textbf{306 to 1,530 images}. Figure~\ref{fig:dist_before} and Figure~\ref{fig:dist_after} show the distribution before and after augmentation.
...
The augmentation balanced the class sizes significantly (from 9--24 to 45--120 per class), enabling the model to learn better representations for under-represented classes.
...
Table~\ref{tab:comparison} compares the two classifiers on the same test set (306 samples).
...
\begin{table}[H]
    ...
    \begin{tabular}{l c c}
        ...
        Training Time (s) & 0.52 & 0.00 \\
        Inference / 1000 samples (ms) & 0.79 & 0.14 \\
        Parameters & $C=10$, $\gamma=\text{scale}$ & $k=1$, cosine \\
        \bottomrule
    \end{tabular}
\end{table}

Both models achieve identical accuracy of \textbf{100.00\%} on the test set. Figure~\ref{fig:comparison} visualizes the differences across multiple metrics.
...
\begin{figure}[H]
    ...
    \caption{PCA projection of face embedding vectors from augmented training dataset}
...
├── dataset/
│   ├── train_raw/                  # Raw extracted faces (306 images)
...
\begin{itemize}
    \item Extracted 306 face images from 22 classroom videos using MTCNN.
    \item Augmented the dataset to 1,530 images with four transformation types.
    \item Trained both SVM (RBF kernel, $C=10$) and KNN (cosine distance, $k=1$) on FaceNet embeddings, with both achieving \textbf{100.00\%} test accuracy.
    \item SVM provides more realistic confidence scores (80.88\%), while KNN offers instant training and faster inference.
